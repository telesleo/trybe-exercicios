...
function nameAndAge(array) {
  const newArray = array.map((book) => ({ name: book.author.name, age: book.releaseYear - book.author.birthYear, }));
...
function oldBooksOrdered(array) {
  const newArray = array.filter((book) => (new Date().getFullYear() - book.releaseYear) > 60);
  return newArray.sort((a, b) => a.releaseYear - b.releaseYear);
}

function fantasyOrScienceFictionAuthors(array) {
  const newArray = array.filter((book) => book.genre === 'Fantasia' || book.genre === 'Ficção Científica');
...
function authorWith3DotsOnName(array) {
  return array.find((book) => book.author.name.split('.').length === 4).name;
}
